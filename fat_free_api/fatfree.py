...
class FatFree:
    def __init__(self, data: dict):
        ...
        self.temperature_coefficient = data['alfa_t[celsius^-1]']
        ...
        self.delta_t = data['delta_t[celsius]']  # °C
        ...
        self.fcn = data['fcn[Mpa]']  # Resistência normalizada de compressão (MPa)
    ...
    def calculate(self):
        ...
        result_dic = {
            'D': self.diameter_outer,
            'L/Ds': self.length / self.diameter,

            'f1_inline': f_inline,
            'f1_crflow': f_crossflow,
            'A1_inline': max(amplitude['lateral']),
            'A1_crflow': max(amplitude['vertical']),
            'delta/D': deflection / self.diameter_outer,
            'Seff/Pe': seff / pcr,
            ...
            'Ai': self.area_intern,
            'A_steel': area_steel_pipe,
            'A_coating': area_coating_pipe,
            'A_concrete': area_concrete_pipe,
            'Ae': self.area_outer
        }
    ...
    def deflection(self, mass_structure_pipe: float, mass_content: float,
        ...
        # Peso Submerso por Unidade de Comprimento (q)
        submerged_weight = pipe_structure_weight - empuxo

        # Solicitacao Axial Efetiva
        seff = effective_axial_force

        # Cálculo da Deflexão Estática
        deflection = c6 * ((submerged_weight * (length_eff_vertical_static**4)) / (ei_steel * (1 + csf))) * (1 / (1 + seff / pcr))

        return deflection, submerged_weight
